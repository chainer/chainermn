...
class MultiNodeBatchNormalizationWithPureNccl(link.Link):
    ...
    def __init__(self, size, comm, decay=0.9, eps=2e-5, dtype=numpy.float32,
        ...
        super(MultiNodeBatchNormalizationWithPureNccl, self).__init__()
        self.comm = comm
        self.avg_mean = numpy.zeros(size, dtype=dtype)
        self.register_persistent('avg_mean')
        self.avg_var = numpy.zeros(size, dtype=dtype)
        ...
        self._func_workspace = MultiNodeBatchNormalizationFunctionWithPureNcclWorkspace()
